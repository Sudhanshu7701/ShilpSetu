...
import OrderCustomizationList from "@/components/OrderCustomizationList";
...
import CustomizationBuilder, { type CustomizationGroup } from "@/components/CustomizationBuilder";
...
interface Order {
  ...
  customizations?: { group: string; label: string; price: number }[] | null;
}
...
const ArtisanDashboard = () => {
  ...
  const handleAddProduct = async (e: React.FormEvent) => {
    ...
    const { error } = await supabase.from("products").insert({
      ...
      customization_options: customizationEnabled ? (customizationGroups as any) : [],
    });
  ...
  const openEditProduct = (p: Product) => {
    ...
    const opts = ((p as any).customization_options || []) as CustomizationGroup[];
    setCustomizationGroups(opts);
    setCustomizationEnabled(opts.length > 0);
    setEditOpen(true);
  };
  ...
  const resetProductForm = () => { setProductForm({ title: "", description: "", price: "", originalPrice: "", region: "", quantity: "1", category: "" }); setUploadedImages([]); setCustomizationEnabled(false); setCustomizationGroups([]); };
  ...
  const renderProductForm = (onSubmit: (e: React.FormEvent) => void, submitLabel: string) => (
    <form onSubmit={onSubmit} className="space-y-4">
      ...
      <CustomizationBuilder
        ...
        onEnabledChange={setCustomizationEnabled}
